...
public class MagnetWorld : MonoBehaviour
{
  ...
  Vector3 CalculateGilbertForce(MagneticObject magnet1, MagneticObject magnet2)
  {
  ...
  void FixedUpdate()
  {
    var magnets = FindObjectsOfType<MagneticObject>();
  ...
  void OnDrawGizmos()
  {
    ...
    for (int i = 0; i < selectedMagnets.Length; i++)
    {
      ...
      if (m1.MagneticPole == MagneticObject.Pole.North)
      {
      ...
      for (int j = 0; j < magCount; j++)
      {
        ...
        if (m2.MagneticPole == MagneticObject.Pole.North)
        {
